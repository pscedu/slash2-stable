 ...
 */
void psc_sem_op(int sem, int lckType)
{
  struct sembuf sbuf;

  if ( ( lckType & LOCK ) ) sbuf.sem_op = -1 ;
  else sbuf.sem_op = 1;

  sbuf.sem_flg = SEM_UNDO;
  sbuf.sem_num = 0;

  psc_dbg("semop() t %d semid %d op %d",
	    lckType, sem, sbuf.sem_op);

  psc_assert(semop(sem, &sbuf, 1) >= 0);

  psc_dbg("semop() t %d semid %d", lckType, sem);
