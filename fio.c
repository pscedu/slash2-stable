...
GROUP_t * find_group(int mype, int *start_pe) 
{ 
 ...
 */
int get_nfiles(IOT_t *iot, GROUP_t *mygroup) { 
  int overflow, nfiles;
...
void init_log_buffers(IOT_t *iot)
{
...
void print_pe_map(struct io_toolbox *iot) { 
...
{ 
  ...
  if ( S_ISDIR(stb.st_mode) ) { 
    ...
    */
    ...
    fprintf(stderr, "Please specify a directory for the test ;%s;",
	    mygroup->test_path);
    exit(1);
  }
 ...
 *   this PE
 */
void init_buffer(struct buffer_t *bdesc, int id) 
{ 
...
int do_creat(struct io_toolbox *iot) 
{ 
  GROUP_t *mygroup = iot->mygroup;
...
int do_open(struct io_toolbox *iot) { 
  GROUP_t *mygroup = iot->mygroup;
  OPLOG_t *op_tmp  = iot->op_log;
  enum CLOCKS clk;
...
int do_unlink(struct io_toolbox *iot) { 
...
int do_io(IOT_t *iot, int op) {
  ...
  for ( iot->bdesc.block_number = 0; 
	...
	if (compare_buffer(&iot->bdesc, &iot->rd_bdesc) != 0) {
	  checksum_ok = NO;
	}
      }
    }
...
int do_write(IOT_t *iot) { 
  return (do_io(iot, 1));
}

int do_read(IOT_t *iot) { 
  return (do_io(iot, 0));
}

int do_trunc(IOT_t *iot) { 
  GROUP_t *mygroup = iot->mygroup;
...
int do_rename(IOT_t *iot) { 
  //struct test_group_t *mygroup = iot->mygroup;
  return 0;
}

int do_link(IOT_t *iot) { 
  //struct test_group_t *mygroup = iot->mygroup;
  return 0;
}

int do_stat(IOT_t *iot) { 
  struct test_group_t *mygroup = iot->mygroup;
  OPLOG_t *op_tmp = iot->op_log;
  ...
  list_for_each(tmp, &groupList) {   
    ...
    for (i=0; i < group->num_pes; i++) { 
      ...
      rc = pthread_create(&group->threads[i].thread_id, NULL, 
			  worker, (void *)&group->threads[i].mype);
      ASSERT(!rc);
    }
...
void printHelp() { 
  fprintf(stderr, "fio [ -d ] [ -n numfiles ] [ -s size ] [ -b blocksz ]\n\t \
[ -p path ] [ -t test_types ] [ -x default_suffix ] [ -o outputfile_path ] [ -i iterations ]\n\n");
  exit(1);
}
