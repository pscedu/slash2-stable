/* $Id: zestPackGeneric.c 1868 2007-10-12 18:52:11Z yanovich $ */
...
#define HDR_SIZE(count) \
    size_round(offsetof (struct zest_msg, buflens[(count)]))

int zest_msg_swabbed(struct zest_msg *msg)
{
        return (msg->magic == __swab32(ZESTRPC_MSG_MAGIC));
}

static void
zest_init_msg (struct zest_msg *msg, int count, int *lens, char **bufs)
{
        char *ptr;
        int   i;

        msg->magic = ZESTRPC_MSG_MAGIC;
        msg->version = ZESTRPC_MSG_VERSION;
        msg->bufcount = count;
        for (i = 0; i < count; i++)
                         ...
                         int count, int *lens, char **bufs)
{
        ...
        reqlen = zest_msg_size (count, lens);
        /* See if we got it from prealloc pool */
        if (req->rq_reqmsg) {
                /* Cannot return error here, that would create
		   infinite loop in zestrpc_prep_req_pool */
                /* In this case zestrpc_prep_req_from_pool sets req->rq_reqlen
	...
	zest_init_msg (req->rq_reqmsg, count, lens, bufs);
                       ...
                       int count, int *lens, char **bufs)
{
        ...
        msg_len = zest_msg_size (count, lens);
        size = offsetof (struct zestrpc_reply_state, rs_msg) + msg_len;
        ZOBD_ALLOC (rs, size);
        if (unlikely(rs == NULL)) {
        ...
        zest_init_msg (&rs->rs_msg, count, lens, bufs);

        //ZESTRPC_RS_DEBUG_LRU_ADD(rs);

        RETURN (0);
}

/* This returns the size of the buffer that is required to hold a zest_msg
...
int zest_unpack_msg(struct zest_msg *m, int len)
{
	 ...
	 * zest_msg may grow, and we'd like to log a version mismatch,
	 * rather than a short message.
	 *
	 */
        required_len = MAX (offsetof (struct zest_msg, version) +
                            sizeof (m->version),
                            offsetof (struct zest_msg, magic) +
                            sizeof (m->magic));
        if (len < required_len) {
        ...
        flipped = zest_msg_swabbed(m);
        if (flipped)
                __swab32s (&m->version);
        else if (m->magic != ZESTRPC_MSG_MAGIC) {
                CERROR("wrong zest_msg magic %#08x\n", m->magic);
                RETURN (-EINVAL);
        }

#if 0
        if ((m->version & ~LUSTRE_VERSION_MASK) != ZESTRPC_MSG_VERSION) {
                CERROR("wrong zest_msg version %#08x\n", m->version);
                RETURN (-EINVAL);
        }
#endif
        ...
        if (len < required_len) {
                /* can't even look inside the message */
                CERROR ("message length %d too small for zest_msg\n", len);
                RETURN (-EINVAL);
        }
...
void *zest_swab_buf(struct zest_msg *msg, int index, int min_size,
		    void *swabber)
{
        void *ptr;

        ptr = zest_msg_buf(msg, index, min_size);
        if (ptr == NULL)
                return NULL;

        if (swabber != NULL && zest_msg_swabbed(msg))
                ((void (*)(void *))swabber)(ptr);

        return ptr;
}

void *zest_swab_reqbuf(struct zestrpc_request *req, int index, int min_size,
                         void *swabber)
{
        LASSERT_REQSWAB(req, index);
        return zest_swab_buf(req->rq_reqmsg, index, min_size, swabber);
}

void *zest_swab_repbuf(struct zestrpc_request *req, int index, int min_size,
                         void *swabber)
{
        LASSERT_REPSWAB(req, index);
        return zest_swab_buf(req->rq_repmsg, index, min_size, swabber);
}
