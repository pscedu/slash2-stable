...
push(@body, "SVNweb:      \thttp://frodo.psc.edu/svnweb/main/revision/?rev=$rev\n");
push(@body, " (port FWD): \thttp://localhost:8080/svnweb/main/revision/?rev=$rev\n");
push(@body, "\n");
if (@$add) {
    my @adds = sort @$add;
    push(@body, "Added:\n");
...
foreach my $project (@project_settings_list)
  {
    my ($j, @add_t, @del_t, @mod_t, @dir_t);

    populate_list($project->{match_re}, \@dirschanged_orig, \@dir_t);
    populate_list($project->{match_re}, \@adds, \@add_t);
    populate_list($project->{match_re}, \@dels, \@del_t);
    populate_list($project->{match_re}, \@mods, \@mod_t);

    next unless @add_t or @del_t or @mod_t or @dir_t;

    my @body = @{ makebody(\@add_t, \@del_t, \@mod_t) };
    ...
    my $xact  = $project->{xact};
    my $subject;

    if ($commondir) {
	$subject = $commondir;
    } else {
	$subject = $dirschanged[0];
    }
    if ($subject =~ /\w+/) {
	    $subject = "SVN Commit: $&";
    } else {
	    $subject = "SVN Commit: $subject";
    }
    $subject = "SVN Commit: $project->{name}";
...
sub new_project
{
  return
	 {name		  => 'projects',
	  email_addresses => [qw(advsys-svn@psc.edu)],
	  from_address    => '',
	  hostname        => '',
	  log_file        => '',
	  match_regex     => '^projects/(?!zest|slash_nara)\b',
	  reply_to        => '',
	  subject_prefix  => ''},
	 {name		  => 'sites',
	  email_addresses => [qw(advsys-svn@psc.edu)],
	  ...
	  match_regex     => '^sites\b',
	  reply_to        => '',
	  xact		  => sub { system("ssh quipu updateslweb"); },
	  subject_prefix  => ''},
	 {name            => 'zest',
	  email_addresses => [qw(zest-devel@psc.edu)],
	  from_address    => '',
	  hostname        => '',
	  log_file        => '',
	  match_regex     => '^projects/zest\b',
	  reply_to        => '',
	  subject_prefix  => ''},
	 {name            => 'slash',
	  email_addresses => [qw(slash2-devel@psc.edu)],
	  from_address    => '',
	  hostname        => '',
	  log_file        => '',
	  match_regex     => '^projects/slash_nara\b',
	  reply_to        => '',
	  subject_prefix  => ''};
}
