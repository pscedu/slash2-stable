...
hash_table_insert (table t, void *value, void *comparator)
{
        ...
        entry->value=value;
        entry->key=k;
        if ((unsigned int)t->number_of_entries > t->size) 
                resize_table(t,t->size*2);
}
 ...
 */
void 
hash_iterate_table_entries(table t,
 ...
 */
void 
hash_filter_table_entries(table t, int (*handler)(void *, void *), void *arg)
{
        unsigned int i;
 ...
 */
void 
hash_destroy_table(table t,void (*thunk)(void *))
{
        table_entry j,next;
        unsigned int i;
        for (i=0;i<t->size;i++)
