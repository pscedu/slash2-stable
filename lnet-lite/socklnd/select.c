...
{
        CDEBUG(D_NET, "remove %d mgr=%p disabled=%d", 
               i->fd, i->argument, i->disabled);
        i->disabled=1;
}
...
static int
prepare_fd_sets(io_handler *ioh, fd_set *r, fd_set *w, fd_set *e)
{
    io_handler j;
    io_handler *k;
    int max = 0;
...
static int 
execute_callbacks(io_handler ioh, fd_set *r, fd_set *w, fd_set *e)
{
    io_handler j;
    int n = 0, t;

    //    for (j = io_handlers; j; j = j->next) {
    for (j = ioh; j; j = j->next) {
            CDEBUG(D_NET, "look for io fd %d mgr=%p disabled=%d", 
                   j->fd, j->argument, j->disabled);
        ...
        if (FD_ISSET(j->fd, r) && (j->type & READ_HANDLER)) {
                CDEBUG(D_NET, "got io for fd %d mgr=%p disabled=%d", j->fd, j->argument, j->disabled);
 ...
 *   functions, if the kernel indicates there is io available.
 */
void select_timer_block(when until, io_handler *ioh)
{
    fd_set fds[3];
    struct timeval timeout;
...
again:
    ...
    if (nready > 0)
            execute_callbacks(*ioh, &fds[0], &fds[1], &fds[2]);
}
